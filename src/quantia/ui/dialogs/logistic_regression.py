...
class LogisticRegressionDialog(BaseAnalysisDialog):
    ...
    def generate_code(self) -> str:
        ...
        if show_plots:
            ...
            code.append("    axes[0].plot([0, 1], [0, 1], color=c_alt, lw=2, linestyle='--')")
            code.append("    axes[0].set_xlim([0.0, 1.0])")
            code.append("    axes[0].set_ylim([0.0, 1.05])")
            code.append("    axes[0].set_xlabel('False Positive Rate')")
            ...
            code.append("    sns.heatmap(cm, annot=True, fmt='d', cmap=cmap, ax=axes[1], cbar=False)")
