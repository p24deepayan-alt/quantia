...
class AnovaModelComparisonDialog(BaseAnalysisDialog):
    ...
    def generate_code(self) -> str:
        ...
        code = [
            ...
            "    html += '</table>'",
            "    ",
            "    # Summary Card with conditional formatting",
            "    p_val = anova_results['Pr(>F)'].iloc[1]",
            "    bg_color = '#F0FDF4' if pd.notna(p_val) and p_val < 0.05 else '#FEF2F2'",
            "    border_color = '#22C55E' if pd.notna(p_val) and p_val < 0.05 else '#EF4444'",
            "    text_color = '#166534' if pd.notna(p_val) and p_val < 0.05 else '#991B1B'",
            "    ",
            "    html += f'<div style=\"margin-top:16px; padding:12px; background:{bg_color}; border-left:4px solid {border_color}; color:{text_color}; font-family:Segoe UI,sans-serif;\">'",
