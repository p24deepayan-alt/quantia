    ...
    def generate_code(self) -> str:
        ...
        if show_plots:
            if self._is_plotly():
                ...
                code.append("    fig_avp = go.Figure()")
                code.append("    fig_avp.add_trace(go.Scatter(x=y, y=results.fittedvalues,")
                code.append("                                  mode='markers', opacity=0.5, name='Predictions'))")
                code.append("    min_val = min(y.min(), results.fittedvalues.min())")
                code.append("    max_val = max(y.max(), results.fittedvalues.max())")
                code.append("    fig_avp.add_trace(go.Scatter(x=[min_val, max_val], y=[min_val, max_val],")
                code.append("                                 mode='lines', name='Perfect Prediction', line=dict(color='red', dash='dash')))")
                code.append("    fig_avp.update_layout(title='Actual vs. Predicted',")
                code.append("                          xaxis_title='Actual', yaxis_title='Predicted')")
                code.append("    if 'show_plotly' in globals():")
                code.append("        html_output += f'<div style=\"margin-top:24px; text-align:center;\">{fig_avp.to_html(include_plotlyjs=\"cdn\", full_html=False)}</div>'")
                ...
                code.append("    ax.scatter(y, results.fittedvalues, alpha=0.5)")
                code.append("    min_val = min(y.min(), results.fittedvalues.min())")
                code.append("    max_val = max(y.max(), results.fittedvalues.max())")
                code.append("    ax.plot([min_val, max_val], [min_val, max_val], color='red', linestyle='--', lw=2)")
